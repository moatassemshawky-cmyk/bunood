...
function sessionCookie(token: string): string {
  ...
  return `bn_contractor_session=${token}; HttpOnly; Secure; SameSite=Strict; Path=/; Expires=${expires}`;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO contractor_sessions (contractor_id, token, expires_at) VALUES ($1,$2,$3)`,
